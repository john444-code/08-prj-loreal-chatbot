...
async function sendMessage(userText) {
  if (!WORKER_URL) {
    throw new Error("Cloudflare Worker URL is not set. Please add WORKER_URL to secrets.js.");
  }
  ...
  try {
    ...
    console.error(error);
    addChatMessage("assistant", "Sorry, I couldn't connect to the L'Oréal assistant right now. Please try again in a moment.");
  } finally {
...
function initializeChat() {
  ...
  addChatMessage("assistant", "Bonjour! I'm your L'Oréal product advisor. Ask me about products, routines, skincare, haircare, makeup, or fragrances.");
}
...
chatForm.addEventListener("submit", async (event) => {
  ...
  try {
    ...
    addChatMessage("assistant", "The chatbot is not ready yet. Please add your deployed Cloudflare Worker URL to secrets.js.");
  }
});
